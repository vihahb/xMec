    ...
    private String getNewsBoxFromPageHTML(Document document) {
        try {
            ...
            if (document.select("div.journal-content-article").size() > 0) {
                ...
                Element titleElement = document.select("div.journal-content-article h2").get(0);
                String html = titleNewsElement.outerHtml() + shortDes.ownText() + titleElement.outerHtml();
